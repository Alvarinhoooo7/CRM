...
function iniciarSesion(email, clave) {
  var props = PropertiesService.getScriptProperties();
  var esperadoEmail = props.getProperty('ACCESO_EMAIL');
  var esperadoHash = props.getProperty('ACCESO_HASH');

  if (!esperadoEmail || !esperadoHash) {
    return { ok: false, token: '',
             mensaje: 'El acceso no esta configurado. Ejecute configurarAcceso_() una vez ' +
                      'desde el editor de Apps Script.' };
  }

  var emailNormalizado = String(email || '').toLowerCase().trim();
  var coincide = (emailNormalizado === esperadoEmail) &&
